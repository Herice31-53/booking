...
<section class="booking_section">
	<div class="appointment_tab" style="margin: auto; overflow-x: auto;overflow-y: visible;" id="loading">
		<div class="loadingGif" id="loadingGif" style="display: none;margin-left: auto;margin-right: auto;justify-content: center;"><img src="Design/images/ajax_loader_gif.gif"></div>
	</div>
				...
				try
				{
					...
					if($client_count >0){
						if($client_result[0]['active'] ==1)
						{
							...
							$stmt_appointment = $con->prepare("insert into appointments(appointment_id,date_created, client_id, employee_id, start_time, end_time_expected ) values(?,?, ?, ?, ?, ?)");
...
http://localhost:8080/BarbershopWebsite/cancel.php?email='.$client_email.'&hash='.$client_result[0]["hash"].'&apid='.$apid.'';
...
http://localhost:8080/BarbershopWebsite/verify.php?email='.$client_email.'&hash='.$hash.'&apid='.$apid.'';

							$headers = 'From:noreply@BarbershopWebsite.com' . "\r\n"; // Set from headers
